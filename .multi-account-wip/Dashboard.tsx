...
/** One dashboard, parameterized by which account it's showing — the primary
 * (yours, at "/") when accountToken is omitted, or a specific friend/client
 * account (at "/a/[token]") when provided. Everything below scopes itself to
 * whichever account the API resolves, including the desk key used for
 * pause/resume/flatten and where it's remembered in this browser. */
export function Dashboard({ accountToken }: { accountToken?: string }) {
  const deskKeyStorageKey = `theonexus_desk_key_${accountToken ?? "primary"}`;
  ...
  const load = useCallback(async () => {
    try {
      const url = accountToken ? `/api/dashboard?account=${encodeURIComponent(accountToken)}` : "/api/dashboard";
  ...
  const flatten = useCallback(
    async (key: string, positionId?: string): Promise<{ text: string; error: boolean }> => {
      if (!data) return { text: "Dashboard not loaded yet.", error: true };
      try {
        const res = await fetch("/api/engine/flatten", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ deskKey: key, positionId, accessToken: data.account.accessToken }),
        });
        ...
        return {
          text: `Flattened ${closedCount} position${closedCount === 1 ? "" : "s"} at market — net ${netTotal >= 0 ? "+" : ""}$${netTotal.toFixed(2)}.`,
          error: false,
        };
      } catch {
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4 px-4 py-6 sm:px-6">
      <header className="flex flex-col gap-1">
        <h1 className="text-lg font-semibold tracking-tight text-foreground">
          ...
          {!data.account.isPrimary && (
            <span className="ml-2 rounded-full border border-panel-border/60 bg-black/20 px-2 py-0.5 text-xs font-normal text-muted">
              {data.account.name}
            </span>
          )}
        </h1>
        ...
        <p className="text-xs text-muted">
          mode: {data.status.mode} (simulated — no real money) · strategy: 1-minute ORB (Tokyo, Shanghai,
          London, New York opens) with VWAP bias
        </p>
      ...
      <Panel
        title="Risk limits"
        subtitle={`$${data.risk.maxLossFromPeak.toLocaleString()} max drawdown from peak equity · $${data.risk.dailyLossLimit.toLocaleString()} max loss per trading day (6pm ET – 5pm ET)`}
      ...
      >
        {data.risk.maxLossBreached && (
          <div className="mb-3 rounded-md border border-danger bg-danger/10 px-3 py-2 text-sm text-danger">
            ...
            {data.risk.maxLossFromPeak.toLocaleString()}. All new entries are blocked for this account until
            this is acknowledged in Engine controls below.
          </div>
        )}
